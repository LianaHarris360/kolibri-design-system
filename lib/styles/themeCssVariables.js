...
import { flattenThemeTree } from './cssVariableNamingRules';
...
export function generateThemeCssVariables() {
  ...
  const collect = (prefix, tree) => {
    for (const [name, value] of flattenThemeTree(prefix, tree)) {
      setVariable(variables, name, value);
    }
  };
  collect('palette', globalThemeState.colors.palette);
  collect('brand', globalThemeState.colors.brand);
  collect('tokens', globalThemeState.tokens);
  return variables;
}
