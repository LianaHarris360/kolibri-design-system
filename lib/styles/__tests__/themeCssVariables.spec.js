...
// The fallback is optional so that every themed var() is checked, including one
// written without a fallback. `[^)]*` deliberately does not parse a nested or
// function-valued fallback, so one fails the check below rather than being accepted
const THEMED_VAR = /var\(\s*(--(?:tokens|brand|palette)-[\w-]+)\s*(?:,\s*([^)]*))?\)/g;

// Every themed var() shipped in lib/, with the fallback it was written with
function themedVarUsages() {
  const files = require('glob').sync('**/*.{vue,scss,css}', {
    cwd: LIB_DIR,
    ignore: ['KIcon/precompiled-icons/**', 'keen/**'],
  });
  const usages = [];
  for (const file of files.sort()) {
    const source = fs.readFileSync(path.join(LIB_DIR, file), 'utf8');
    for (const [, name, fallback] of source.matchAll(THEMED_VAR)) {
      usages.push({ file, name, fallback: fallback && fallback.trim() });
    }
  }
  return usages;
}
...
describe('themeCssVariables', () => {
  ...
  describe('globals.scss', () => {
    ...
    it('uses only the var() fallbacks listed above', () => {
      // intentional flat matching: a nested or function-valued fallback fails here
      // rather than being parsed, prompting an update to the table
  ...
  describe('themed var() fallbacks in lib/', () => {
    it('falls back to the value the theme emits, in every themed var() in lib/', () => {
      const variables = themeCssVariables.generateThemeCssVariables();
      const usages = themedVarUsages();
      // without this, the test would pass if the file search found nothing
      expect(usages).not.toHaveLength(0);
      // `undefined` on either side is a mismatch: a var() written without a
      // fallback, or one naming a variable the theme never emits
      const written = usages.map(
        ({ file, name, fallback }) => `${file}: ${name}, ${fallback && fallback.toLowerCase()}`,
      );
      const emitted = usages.map(
        ({ file, name }) => `${file}: ${name}, ${variables[name] && variables[name].toLowerCase()}`,
      );
