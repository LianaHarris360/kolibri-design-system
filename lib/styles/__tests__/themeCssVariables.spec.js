...
// `[^)]*` deliberately does not parse a nested or function-valued fallback, so one
// fails the check below rather than being silently accepted
const THEMED_VAR_WITH_FALLBACK = /var\(\s*(--(?:tokens|brand|palette)-[\w-]+)\s*,\s*([^)]*)\)/g;

// Every themed var() fallback shipped in lib/
function themedVarFallbacks() {
  ...
  for (const file of files.sort()) {
    ...
    for (const [, name, fallback] of source.matchAll(THEMED_VAR_WITH_FALLBACK)) {
      usages.push({ file, name, fallback: fallback.trim() });
    }
  }
  return usages;
}
...
const palette = require('../colorsMaterial');
...
describe('themeCssVariables', () => {
  ...
  describe('themed var() fallbacks in lib/', () => {
    it('falls back to the value the theme emits, in every source that has one', () => {
      const variables = themeCssVariables.generateThemeCssVariables();
      const usages = themedVarFallbacks();
      expect(usages).not.toHaveLength(0);
      const written = usages.map(
        ({ file, name, fallback }) => `${file}: ${name}, ${fallback.toLowerCase()}`,
      );
    ...
    it('compares fallbacks without regard to hex case', () => {
      const variables = themeCssVariables.generateThemeCssVariables();
      expect(variables['--tokens-surface']).toBe('#FFFFFF');
      expect(themedVarFallbacks().map(({ fallback }) => fallback)).toContain('#ffffff');
    });
  });
